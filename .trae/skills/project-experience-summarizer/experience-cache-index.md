...
| L0-008 | #29 | 流程问题 | 通用 | AI幻觉, 测试跳过, 静态分析 | 0 | - | 检查是否调用了强制工具，是否有实际运行证据 |
| L0-009 | #30 | 流程问题 | Web游戏 | 视觉测试, PNG截图, MD代替PNG | 0 | - | 检查screenshots目录下是否有.png文件（不是.md文件） |
| L0-010 | #31 | 流程问题 | 通用 | VT+FPT, 浏览器全屏, 阻塞式测试 | 0 | - | 检查VT和FPT是否合并执行，是否用浏览器全屏打开，每步是否截图验证 |
...
| AI幻觉 | L0-008 | #29 | AI跳过测试步骤，静态分析代替动态测试 |
| 测试跳过 | L0-008 | #29 | AI声称测试完成但未实际执行 |
| 静态分析 | L0-008 | #29 | 仅凭代码分析声称测试通过 |
| VT+FPT | L0-010 | #31 | VT和FPT必须合并执行，阻塞式流程 |
| 浏览器全屏 | L0-010 | #31 | 必须用浏览器全屏打开，全屏截图 |
| 阻塞式测试 | L0-010 | #31 | 每一步必须截图验证后才能继续 |
